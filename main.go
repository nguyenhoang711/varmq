...
func NewWorker[T, R any](wf WorkerFunc[T, R], config ...Config) Queues[T, R] {
...
// if concurrency is less than 1, it will use the number of CPUs as the concurrency
func NewEWorker[T any](wf WorkerErrFunc[T], config ...Config) Queues[T, any] {
...
// if concurrency is less than 1, it will use the number of CPUs as the concurrency
func NewVoidWorker[T any](wf VoidWorkerFunc[T], config ...Config) Queues[T, any] {
	worker := newWorker[T, any](wf, config...)
	return newQueues[T, any](worker)
}
